...
3. 답변 잠금, 공개, 선택, 주문 신청, 미리보기, 주문 완료, 주문 내역 상태가 한눈에 이해되어야 한다.
...
### 주문 기록 선택 화면
- `/export` 화면은 기록 선택과 주문 내역을 탭으로 구분한다.
- 기록 항목을 체크해 선택한다.
...
### 주문 내역 화면
- 진행 중인 주문 예약과 완료된 주문을 생성일 내림차순으로 보여준다.
- PREVIEWED 주문은 "주문 예약 단계" 배지와 "예약 내용 보기" 버튼을 제공한다.
- COMPLETED 주문은 "주문 완료" 배지와 "주문 내용 보기", "삭제" 버튼을 제공한다.
- 완료된 주문 상세에서는 저장된 스냅샷 내용을 읽기 전용으로 보여주고 JSON 다운로드, 텍스트 다운로드, 삭제 버튼을 제공한다.
- 완료된 주문은 항목이나 내용을 수정할 수 없으며, 수정이 필요하면 새 주문을 만든다.
- 주문 항목 수, 주문 번호, 기준 시각을 함께 보여준다.
- 주문 내역이 없으면 기록 선택으로 주문 신청을 시작하라는 빈 상태를 표시한다.
- 상태는 색상만으로 표현하지 않고 배지 텍스트와 설명 문구를 함께 제공한다.
